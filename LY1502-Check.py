...
# 定义常量
paramfiles = {'parameter': u'NLY1502出厂检测模版.xlsx'}
...
def write645data(ldata, index, lists):
    ...
    if ldata['op'].replace(' ', '') != 'Set645':
        return False
    if isinstance(ldata['id'], str) == False:
        return False
    ids = ldata['id'].split(',')
    if len(ids) == 0:
        return False
...
def initModel():
    ...
    logging.basicConfig(format='%(asctime)s %(levelname)s:%(message)s', datefmt='%y/%m/%d %H:%M:%S',
                        filename=rootdir + '\log\\'+snow+'NLY1502.log', level=logging.DEBUG)
...
def main():
    ...
    HOST, PORT = "192.168.127.25", 11039
    ...
    if checkfile(paramfiles):
        #"D:\python\designMeter\config"
        path =rootdir + "\config"
        # daidlist =  readexcel(path+'/'+ paramfiles['relationship'])
        dllist = readexcel(path+'/' + paramfiles['parameter'], u'出厂检测流程')
        print(len(dllist))
    ...
    if len(test_addr) >= 1:
        ...
        path = rootdir + "\\report\\" + test_addr[0] + '-'+now.strftime('%Y%m%d%H%M%S')+'.xls'
    else:
        path = rootdir + "\\report\\"+ now.strftime('%Y%m%d%H%M%S')+'.xls'
    SaveAsExcel(path, dllist)
    if totalresult(dllist):
        logging.info(u'出厂检测流程,合格!')
    else:
        logging.info(u'出厂检测流程,不合格!')
