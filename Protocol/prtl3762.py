...
def changefn(data):
    ...
    for j in range(0, 8):
        if data2[j] == '1':
            ...
            E = str(D)
            F += ''.join(E)
    for XP in range(0,len(F)):
        StrValue += str( (int(data[2:4],16))*8 + int(F[XP],16))
    return  StrValue
...
def ana(frame,ctrl,data):
    ...
    if message1[1]=='1':
       if message[0]=='0':
           ...
           StrValue.append('应用数据域 AFN = '+ data[36:38] +',' 'FN = '+ changefn(data[38:42]))
           data1 = data[36:]
           abc = DI(ctrl,data1)
           StrValue.extend(abc)
       else:
           ...
           StrValue.append('应用数据域 AFN = ' + data[12*(int( message[0])+1):12*(int( message[0])+1)+2] + ',' 'FN = ' + changefn(data[12*(int( message[0])+1)+2:12*(int( message[0])+1)+6]))
           data1=data[12*(int( message[0])+1):]
           StrValue.append(DI(ctrl,data1))
    else:
        StrValue.append('应用数据域:  AFN = '+ data[12:14] +',' 'FN = '+ changefn(data[14:18]))
        data1 = data[12:]
        abc=DI(ctrl,data1)
        StrValue.extend(abc)
    return (StrValue)
...
def FieldParsing13762_AFN00(data):
     ...
     if len(data) > 4:
         ...
         StrValue.append('数据单元 = ' + '无')
     if data[:4]=='0100':
        StrValue.append('-------- 确认/否认：确认 --------')
        StrValue.append('确认')
        return StrValue
     elif data[:4]=='0200':
        StrValue.append('-------- 确认/否认：否认 --------')
        StrValue.append('否认')
        return (StrValue)
     else:
...
def FieldParsing13762_AFN01(data):
    ...
    if len(data) > 4:
        StrValue.append('数据单元 = ' + data[4:])
    else: StrValue.append('数据单元 = ' + '无')
    if data[:4] == '0100':
        StrValue.append('-------- 初始化：硬件初始化 --------')
        StrValue.append( '硬件初始化')
        return StrValue
    elif data[:4] == '0200':
        StrValue.append('-------- 初始化：参数区初始化 --------')
        StrValue.append( '参数区初始化(清除所有从节点档案信息)')
        return (StrValue)
    elif data[:4] == '0400':
        StrValue.append('-------- 初始化：数据区初始化 --------')
...
def FieldParsing13762_AFN06b(data):
    ...
    if  data[:4] == '0100':
        StrValue.append('--------主动上报 ：上报从节点信息 --------')
        if len(data[6:]) %18==0:
            for i in range(int(data[4:6])):
                StrValue.append('上报从节点数量 = ' + str(int(data[4:6],16)))
                StrValue.append('从节点地址'+ str(i+1) + ' = '+ reverse(data[6+18*i:18*(i+1)]))
                StrValue.append('从节点类型 '+ str(i+1) + '= '+ data[18*(i+1):18*(i+1)+2] + ' (00H＝透明传输；01H＝DL/T645—1997；02H＝DL/T645—2007；03H＝DL/698；04H—FFH保留)')
                StrValue.append('从节点序号'+ str(i+1) + ' = ' +  str(int(reverse(data[18*(i+1)+2:18 * (i + 1) + 6]),16)))
            return StrValue
        else:
            return 'Cannot Parse The data: ' + data
    elif  data[:4] == '0200':
        StrValue.append('--------主动上报 ：上报抄读数据 --------')
        if len(data) > 8:
            ...
            StrValue.append('当前报文本地通信上行时长 = ' + str(int(reverse(data[10:14]),16)))
        ...
        StrValue.append('--------主动上报 ：上报路由工况变动信息 --------')
        if len(data) == 6:
        ...
        StrValue.append('-------- 主动上报 ：上报从节点信息及设备类型 --------')
        if len(data) >= 30:
            StrValue.append('上报从节点数量 = ' + str(int(data[4:6],16)))
            a=data[28:30]
            b = int(data[4:6], 16)
            if a==0:
                for i in range(0,b):
                    StrValue.append('上报从节地址'+ str(i+1) + ' = ' + reverse(data[6+24*i:18+24*i] ))
                    StrValue.append('从节点'+ str(i+1) +'通讯协议类型 = ' + data[18+24*i:20+24*i] + ' （00H＝透明传输；01H＝DL/T645—1997；02H＝DL/T645—2007；03H＝相位识别功能；04HFFH保留）')
                    StrValue.append( '从节点'+ str(i+1) + '序号  = ' + data[20+24*i:24+24*i] )
                    StrValue.append('从节点'+ str(i+1) + '设备类型 = ' + data[24+24*i:26+24*i] + '（00H＝采集器；01H＝电能表；02H—FFH 保留）')
                    StrValue.append( '从节点'+ str(i+1) + '下接的从节点数量 = ' + data[26+24*i:28+24*i] )
                    StrValue.append( '本文传输的的从节点数量 = ' + data[28+24*i:30+24*i] )

                    return StrValue
            else:
                for i in range(0,b):
                    StrValue.append( '上报从节地址'+ str(i+1) + ' = ' + reverse(data[6:18] ))
                    StrValue.append('从节点'+ str(i+1) +'通讯协议类型 = ' + data[18:20] + '（00H＝透明传输；01H＝DL/T645—1997；02H＝DL/T645—2007；03H＝相位识别功能；04HFFH保留）')
                    StrValue.append( '从节点'+ str(i+1) + '序号  = ' + data[20:24] )
                    StrValue.append('从节点'+ str(i+1) + '设备类型 = ' + data[24:26] + '（00H＝采集器；01H＝电能表；02H—FFH 保留）')
                    StrValue.append( '从节点'+ str(i+1) + '下接的从节点数量 = ' + data[26:28] )
                    StrValue.append('本文传输的的从节点数量 = ' + data[28:30] )
                    if data[28:30] != '00':
                        c = int(data[28:30],16)
                        for j in range(0,c):
                             StrValue.append('下接从节点的地址 = ' + reverse(data[30+14*j:42+14*j]))
                             StrValue.append('下接从节点的通信协议类型 = ' + data[42+14*j:44+14*j] + '（00H＝透明传输；01H＝DL/T645—1997；02H＝DL/T645—2007；03H＝相位识别功能；04HFFH保留）')
                    return  StrValue
        else:
            return 'Cannot Parse The data = ' + data
    elif data[:4] == '1000':
        StrValue.append('-------- 主动上报 ：上报从节点事件 --------')
        if len(data) >= 6:
...
def FieldParsing13762_AFN10a(data):
    ...
    if  data[:4] == '0100':
        StrValue.append('-------- 路由查询类 ：从节点数量 --------')
        if  len(data) ==4:
            return  StrValue
        else:
            return 'Cannot Parse The data = ' + data
    elif  data[:4] == '0200':
        StrValue.append('-------- 路由查询类 ：从节点信息 --------')
        if len(data) == 10:
            StrValue.append('从节点起始序号 = ' + str(int(reverse(data[4:8])),16))
            StrValue.append('从节点数量 = ' + str(int(data[8:],16)))
            return StrValue
        else:
            return 'Cannot Parse The data = ' + data
    elif  data[:4] == '0400':
        StrValue.append('-------- 路由查询类 ：指定从节点的上一级中继路由信息 --------')
        if len(data) == 16:
            StrValue.append('从节点地址 = ' + reverse(data[4:]))
            return StrValue
        else:
            return 'Cannot Parse The data = ' + data
    elif data[:4] == '0800':
        StrValue.append('-------- 路由查询类 ：路由运行状态 --------')
        if len(data) ==4:
            return StrValue
        else:
            return 'Cannot Parse The data = ' + data
    elif data[:4] == '1000':
        StrValue.append('-------- 路由查询类 ：未抄读成功的从节点信息 --------')
        if len(data) ==10:
        ...
        StrValue.append('-------- 路由查询类 ：主动注册的从节点信息 --------')
        if len(data) ==10:
            StrValue.append('从节点起始序号 = ' + str(int(reverse(data[4:8])),16))
            StrValue.append('从节点数量 = ' + str(int(data[8:],16)))
            return StrValue
        ...
        StrValue.append('-------- 路由查询类 ：查询网络规模 --------')
        if len(data) == 4:
            return StrValue
        else:
            return 'Cannot Parse The data = ' + data
    elif data[:4] == '100C':  #F100查询网络规模
        StrValue.append('-------- 路由查询类 ：查询微功率无线从节点信息 --------')
        if len(data) == 4:
            return StrValue
        else:
            return 'Cannot Parse The data = ' + data
...
def FieldParsing13762_AFN10b(data):
    ...
    if  data[:4] == '0100':
        ...
        if len(data) >=26:
            ...
            a=int(data[4:6],16)
            for i in range(0,a):
                 StrValue.append('提供路由的从节点'+ str(i+1)+'地址 = ' + reverse(data[6+16*i:18+16*i]))
                 StrValue.append('提供路由的从节点'+ str(i+1)+'信息 = ' + reverse(data[18+16*i:22+16*i]))
            return StrValue
        else:
            return 'Cannot Parse The data: ' + data
    elif data[:4] == '0800':
        ...
        if len(data) ==10:
            ...
            a = int(data[8:10],16)
            for i in range(0,a):
                 StrValue.append('从节点'+str(1+i) +'地址 = ' + data[10+16*i:22+16*i])
                 d = bin(data[23+16*i])
                 e = d.zfill(10)
                 f = d.replace('0b', '')
                 g=int(f[2:5])
        ...
        if len(data) >= 22:
            ...
            for i in range(0,a):
                ...
                for i in range(len(g)):
                    h = ''.join(g)
                    StrValue.append('从节点'+str(1+i) +'通信协议类型 = ' + h + ' （00H＝透明传输；01H＝DL/T645—1997；02H＝DL/T645—2007；03H＝DL—698；04H-FFH保留）')
                if g[5] == 1:
                    StrValue.append('从节点'+str(1+i) +'相位 = C相')
                if g[6] == 1:
                    StrValue.append('从节点'+str(1+i) +'相位 = B相')
                if g[7] == 1:
                    StrValue.append('从节点'+str(1+i) +'相位 = A相')
                return StrValue
        else:
            return 'Cannot Parse The data: ' + data
    elif data[:4] == '080C':  #F100查询网络规模
        StrValue.append('-------- 路由查询类 ：查询网络规模 --------')
        if len(data)==8:
            StrValue.append('网络规模 = ' + str(int(reverse(data[4:8]),16)))
        else:
            return 'Cannot Parse The data: ' + data
    elif data[:4] == '100C':  #F100查询网络规模
        StrValue.append('-------- 路由查询类 ：查询微功率无线从节点信息 --------')
        if len(data) >= 22:
            ...
            for i in range(int(data[8:10])):
                StrValue.append('从节点'+str(1+i) +'地址 =' + reverse(data[10 + 22 * i:22 + 22 * i]))
                d = bin(data[23 + 16 * i])
                e = d.zfill(10)
                f = d.replace('0b', '')
                g = int(f[2:5])
                ...
                if g[7] == 1:
                    StrValue.append('从节点'+str(1+i) +'相位 = A相')
                StrValue.append('从节点'+str(1+i) +'软件版本信息 = ' + data[26 + 22 * i:32 + 22 * i])
                return StrValue
        else:
            return 'Cannot Parse The data: ' + data
    else:
...
def FieldParsing13762_AFN11a(data):
    ...
    if data[:4] == '0100':
        StrValue.append('-------- 路由设置类 ：添加从节点 --------')
        if len(data) >= 20:
            StrValue.append('从节点数量 = ' + str(int(data[4:6],16)))
            a = int(data[4:6],16)
            for i in range(0,a):
                 StrValue.append('从节点'+str(1+i) +'地址 = ' + reverse(data[6+14*i:18+14*i]))
                 StrValue.append('从节点'+str(1+i) +'通信协议类型 = ' + data[18+14*i:20+14*i] + '（00H＝透明传输；01H＝DL/T645—1997；02H＝DL/T645—2007；03H＝DL—698；04H-FFH保留）')
            return StrValue
        else:
            return 'Cannot Parse The data = ' + data
    elif data[:4] == '0200':
        StrValue.append('-------- 路由设置类 ：删除从节点 --------')
        if len(data) >= 18:
            StrValue.append('从节点数量 = ' + str(int(data[4:6],16)))
            a = int(data[4:6],16)
            for i in range(0,a):
                 StrValue.append('从节点'+str(1+i) +'地址 = ' + data[6+12*i:18+12*i])
            return StrValue
        else:
            return 'Cannot Parse The data: ' + data
    elif data[:4] == '0400':
        StrValue.append('-------- 路由设置类 ：设置从节点固定中继路径 --------')
        if len(data) >= 8:
            StrValue.append('从节点地址 = ' + reverse(data[4:16]))
            StrValue.append('中继级别 = ' + str(int(data[16:18],16)))
            a = int(data[16:18],16)
            for i in range(0,a):
                 StrValue.append('中继从节点'+ str(1+i) + '地址 = ' + reverse(data[18+12*i:30+12*i]))
            return StrValue
        else:
            return 'Cannot Parse The data: ' + data
    elif data[:4] == '0800':
        StrValue.append('-------- 路由设置类 ：设置路由工作模式 --------')
        if len(data) == 10:
            c = trans(data[5])
            StrValue.append('纠错编码 = ' + data[4])
        ...
        StrValue.append('-------- 路由设置类 ：激活从节点主动注册 --------')
        if len(data) == 24:
            StrValue.append('开始时间 = ' + reverse(data[4:16]))
            StrValue.append('持续时间 = ' + str(int(reverse(data[16:20]),16)))
            StrValue.append('从节点重发次数 = ' + str(int(data[20:22],16)))
            StrValue.append('随机等待时间个数 = ' + str(int(data[22:24],16)))
            return StrValue
        else:
            return 'Cannot Parse The data: ' + data
    elif data[:4] == '2000':
        StrValue.append('-------- 路由设置类 ：终止从节点主动注册 --------')
        if len(data) == 4:
            return
        else:
            return 'Cannot Parse The data: ' + data
    elif data[:4] == '080C':  #F100设置网络规模
        StrValue.append('-------- 路由设置类 ：设置网络规模 --------')
        if len(data) == 8:
        ...
        StrValue.append('-------- 路由设置类 ：启动网略维护进程 --------')
        if len(data) == 4:
            return
        else:
            return 'Cannot Parse The data: ' + data
    elif data[:4] == '200C':  # F102启动组网
        StrValue.append('-------- 路由设置类 ：启动组网 --------')
        if len(data) == 4:
            return
        else:
            return 'Cannot Parse The data: ' + data
...
def FieldParsing13762_AFN14b(data):
    ...
    if data[:4] == '0100':
        StrValue.append('-------- 路由数据抄读类 ：路由请求抄读内容 --------')
        if  len(data) >= 10:
        ...
        StrValue.append('-------- 路由数据抄读类 ：路由请求集中器时钟 --------')
        if len(data) == 16:
            StrValue.append('当前时间 = ' + reverse(data[4:16]))
            return StrValue
        else:
            return 'Cannot Parse The data = ' + data
    elif data[:4] == '0400':
        StrValue.append('-------- 路由数据抄读类 ：请求依通信延时修正通信数据 --------')
        if len(data) >= 6:
...
def FieldParsing13762_AFN14a(data):
    ...
    if data[:4] == '0100':
        ...
        if  len(data) == 22:
            ...
            StrValue.append('从节点序号 = ' + str(int(reverse(data[18:22],16))))
            return  StrValue
        else:
            return 'Cannot Parse The data: ' + data
    if data[:4] == '0200':
