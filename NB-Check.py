...
def read645data(ldata, index, lists):
    ...
    if len(ids) == 1:
        ...
        print(anlst[1])
        #IMSI作为文件名
        if ids[1].find('F0000003') >= 0 and len(anlst) == 5:
            test_addr.append(anlst[4])
            print(test_addr)
    if anlst[0] and len(anlst) == 5:
        ldata['real'] = anlst[4]
    else:
...
def initModel():
    ...
    logging.basicConfig(format='%(asctime)s %(levelname)s:%(message)s', datefmt='%y/%m/%d %H:%M:%S',
                        filename='D:\python\designMeter\log\\'+snow+'NB.log', level=logging.DEBUG)


# 定义常量
paramfiles = {'parameter': u'NB模块出厂检测模版.xlsx'}
