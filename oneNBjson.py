...
def procexcel(excellist):
    for itm in excellist:
        ...
        logging.info(temstr)
        print(excellist[itm]['op'])
        print(excellist[itm]['op'].find(CMD_JSON_TRANSET))
        ...
        elif excellist[itm]['op'].find(CMD_JSON_TRANSET) >= 0:
            print('CMD_JSON_TRANSET')
            transsetmdata(excellist[itm], itm, excellist)
            time.sleep(JGTIME)
...
def transmeterdata(ldata, index, lists):
    if isinstance(ldata['op'], str) and ldata['op'] == '':
        print('trans-1')
        return False
    if ldata['op'].replace(' ', '') != CMD_JSON_TRANS:
        print('trans-2')
        return False
    if isinstance(ldata['id'], str) and len(ldata['id'].relace(' ', '')) < 8:
        return False
    ...
    if isinstance(ldata['data'], float):
        ...
        sdata = ldata['data']
    sdd = make645Frame('', M_ADDR, '11', sid, sdata, 1)
    tdcts = {'F0000010': sdd}
    scm = ldata['op'].replace(' ', '')
    # print tdcts
    sTx = makepjson(scm, index, tdcts)
    ...
    while itime < RXOUTTIME:
        time.sleep(0.2)
        if len(public_sRxBuff) > 0:
            slst = isReturn(public_sRxBuff[0], CMD_JSON_TRANS, ['F0000010'])
            logging.info('Rx:' + public_sRxBuff[0])
            del(public_sRxBuff[0])
        if len(slst) > 0 and slst[0]:
            logging.info('wait:' + str(itime))
    ...
    print(slst)
    if len(slst) == 3:
        datalist.append(slst[2]['F0000010'])
        # ldata['real'] = datalist[0]
        sd = is645Return(datalist[0], M_ADDR, '11', sid)
        print(sd)
        if sd[0] and len(sd) == 5:
            ldata['real'] = sd[4]
        else:
            ldata['real'] = datalist[0]
# 判断结论
...
# 透传设置电表数据
def transsetmdata(ldata, index, lists):
    if isinstance(ldata['op'], str) and ldata['op'] == '':
        return False
    # print 'TransSet-1'
    if ldata['op'].replace(' ', '') != CMD_JSON_TRANSET:
        return False
    # print 'TransSet-2'
    if isinstance(ldata['id'], str) and len(ldata['id'].relace(' ', '')) < 8:
        return False
    ...
    print(sdata)
    sdata = '3733333333333333' + sdata
    sdd = make645Frame('', M_ADDR, '14', Reversal(Add33(sid)), sdata, 0)
    tdcts = {'F0000010': sdd}
    print(tdcts)
    scm = u'Trans'
    sTx = makepjson(scm, index, tdcts)
    ...
    if len(slst) == 3:
        datalist.append(slst[2]['F0000010'])
        # ldata['real'] = datalist[0]
        sd = is645Return(datalist[0], M_ADDR, '14', sid)
        print(sd)
        if sd[0] and len(sd) == 5:
            ldata['real'] = sd[4]
        else:
...
# 定义常量
paramfiles = {'parameter': u'单相表NB模块.xlsx'}
